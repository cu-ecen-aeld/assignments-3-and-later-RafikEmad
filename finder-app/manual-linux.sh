...
echo "Creating the necessary base directories for the root filesystem"
mkdir -p ${OUTDIR}/rootfs/{bin,dev,etc,home,lib,lib64,proc,sbin,sys,tmp,usr,var}/{bin,lib,sbin} ${OUTDIR}/rootfs/var/log
...
echo "Installing BusyBox to root filesystem"
...
cd ${OUTDIR}/rootfs
# Check the installed binary in rootfs
${CROSS_COMPILE}readelf -a bin/busybox | grep "program interpreter"
${CROSS_COMPILE}readelf -a bin/busybox | grep "Shared library"

echo "Adding necessary library dependencies to rootfs"
...
# Assuming the source is available under some directory, replace with actual path
cd ${FINDER_APP_DIR}
make clean
...
# Set the setuid bit for the busybox binary
echo "Setting setuid for busybox"
